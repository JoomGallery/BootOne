...
<?php if($this->params->get('show_category_button')): ?>
      <div class="btn-group">
        <button type="button" class="btn btn-default" onclick="javascript:location.href='<?php echo JRoute::_('index.php?view=editcategory'.$this->slimitstart, false); ?>';">
...
<?php       endif ?>
            ...
            <th class="nowrap hidden-xs" width="30%">
...
<?php   $i = 0;
        ...
        foreach($this->items as $i => $item):
          ...
          <tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->parent_id;?>" item-id="<?php echo $item->cid?>" parents="<?php echo $parentsStr; ?>" level="<?php echo $item->level; ?>">
            <td class="order nowrap center hidden-xs">
<?php       if($canChange):
              $disableClassName = '';
              $disabledLabel    = '';
              if(!$saveOrder):
            ...
            <td class="center hidden-xs hidden">
